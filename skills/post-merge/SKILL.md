...
Determine whether a skill named `handoff` is available.

If the `handoff` skill is available:

- invoke the `handoff` skill
- allow the `handoff` skill to generate the session handoff
- do not generate another handoff or duplicate its output
- continue with this workflow after the `handoff` skill has completed

If the `handoff` skill is not available:

Generate a concise session summary including:
...
When another dedicated skill is available for a responsibility, prefer using that skill instead of reimplementing its behavior.
...
- a handoff exists, generated either by the `handoff` skill or by this workflow if no such skill is available
